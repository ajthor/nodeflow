...
exports.writeTests = function (callback) {
	...
	var watcher = fs.watch(path.join(process.cwd(), '/test'));
...
// gitAdd Function
// ----------------
// The add function checks the differences between the files you 
// have just modified and displays the diff on the terminal.
exports.gitAdd = function (callback) {
	var git = exec('git add -i', function (code) {
		...
		inquirer.prompt(
			...
			function (answers) {
				if (!answers.message) {
					console.log('Must enter a git commit message.');
...
// Does what it says.
exports.gitCommit = function (obj, callback) {
	if (obj.message) {
		var git = exec('git commit -m ' + obj.message, function (code) {
			callback(null);
		});
	}
	else {
		console.log('Could not commit. No message specified.');
		callback(null);
	}
};
